...
func (at *AbsText) ExistInMongodb() (b bool) {
	...
	result := coll.FindOne(context.TODO(), bson.M{at.Tag(): at.Value()})
...
func (at *AbsText) Dump() (err error) {
	...
	if at.ExistInMongodb() {
		...
		_, err = coll.ReplaceOne(context.TODO(), bson.M{at.Tag(): at.Value()}, *at) // 更新
	} else {
		// 不存在则插入
		coll := mongodb.Get(Database, Collection)    // 获取 media collection 的句柄
...
func (at *AbsText) Load() (err error) {
	...
	if at.ExistInMongodb() {
		...
		// 加载数据
		absText := AbsText{}
		err = coll.FindOne(context.TODO(), bson.M{at.Tag(): at.Value()}).Decode(&absText)
		*at = absText
		if err != nil {
			logger.Error.Println(err.Error())
...
func (at *AbsText) Delete() (err error) {
	...
	if at.ExistInMongodb() {
		coll := mongodb.Get(Database, Collection) // 获取 collection 的句柄
		// 删除
		_, err = coll.DeleteOne(context.TODO(), bson.M{at.Tag(): at.Value()})
		if err != nil {
			logger.Error.Println(err.Error())
