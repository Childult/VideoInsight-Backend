...
from text_analysis.api import text_summarize, cal_text_similarity
...
class Test(TestCase):
    def test_text_summarize(self):
        ...
        for data in test_data:
            res = text_summarize(data)
            self.assertGreaterEqual(len(res), 1)
            for seg in res:
                self.assertFalse(seg.endswith(','))

    def test_cal_text_similarity(self):
        test_data = {
            'similar': [
                (
                    '你好,世界。',
            ...
            'not similar': [
                (
                    '如果最小键值元素拥有最高的优先级，那么这种优先队列叫作升序优先队列（即总是先删除最小的元素），类似的，如果最大键值元素拥有最高的优先级，那么这种优先队列叫作降序优先队列',
                    '首先会对提交的作品重点进行创新性分析。若创新性分析不能通过，则作品不被接受。'
                ),
                (
                    '题材符合导向性，能够围绕端云结合的人工智能相关技术，在一定的程度上解决某种问题，但给出的解决方案体现出的改进效果差强人意',
                    '在二叉堆中，每个结点最多有两个孩子结点，在实际应用中，二叉堆已经足够满足需求，因此接下来主要讨论二叉最小堆和二叉最大堆'
                ),
            ],
        }
        for data in test_data['similar']:
            self.assertGreater(cal_text_similarity(data[0], data[1]), 0.5)
        for data in test_data['not similar']:
            self.assertLess(cal_text_similarity(data[0], data[1]), 0.6)
